...
rlosFields = {"OFFSETA":[2.0, "Observer offset above surface"],
              ...
              "AZIM1":[0.0, "Left azimuth"],
              "AZIM2":[360.0, "Right azimuth"],
...
def _addDoubleField(targetTable, fieldsToAdd):
    ...
    try:
        ...
        for currentField in fieldsToAdd:
            if currentField in existingField:
                arcpy.AddWarning("Field {0} is already in {1}".format(currentField, targetTable))
            else:
                fName = str(fieldsToAdd[currentField])
                #fDefault = float(fieldsToAdd[currentField][0])
                fAlias = str(fieldsToAdd[currentField][1])
...
def addLLOSFields(inputObserverTable,
    ...
    try:
        ...
        outputObserverTable = _addDoubleField(inputObserverTableTable,
                                      llosFields)
