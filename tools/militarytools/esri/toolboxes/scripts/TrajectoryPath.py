...
def SurfaceRange(d, initialVelocityMPS, elevationAngleRadians, gravitationalConstant):
...
def TimeToRange(h, initialVelocityMPS, elevationAngleRadians, gravitationalConstant):
    # time to current position from origin point
    t = (initialVelocityMPS * math.sin(elevationAngleRadians)/gravitationalConstant) + math.sqrt(math.pow(initialVelocityMPS,2) * math.pow(math.sin(elevationAngleRadians),2) + (2 * gravitationalConstant * math.fabs(h)))/ gravitationalConstant
    return t
...
def computeTrajectory(inputFeature, initialVelocityMPS, elevationAngleDegrees, \
                       azimuthAngleDegrees, analysisType, inputSurface, outFeature):
    ...
    try:
        ...
        if debug == True:
            ...
            arcpy.AddMessage("analysisType: " + str(analysisType))
            arcpy.AddMessage("inputSurface: " + str(inputSurface))
            arcpy.AddMessage("outFeature: " + str(outFeature))
   
        if arcpy.Exists(outFeature):
            arcpy.AddMessage("Output already exists, deleting: " + str(outFeature))
            arcpy.Delete_management(outFeature)
                                                     
        env.overwriteOutput = True
        scratch = env.scratchWorkspace
    
        commonSpatialReference = arcpy.Describe(inputSurface).spatialReference;
        ...
        # create output layer
        arcpy.CreateFeatureclass_management(os.path.dirname(outFeature), os.path.basename(outFeature), \
                                            "POLYLINE","","ENABLED","ENABLED", env.outputCoordinateSystem)
        ...
        addRows = arcpy.da.InsertCursor(outFeature,["SHAPE@", "MaxHeight", "Range", "TimeImpact", \
                                                    "Bearing","ElevAngle","InitV"])
        ...
        for obsOID in list(observersProj.keys()):
            ...
            if (stepRange > 0):
                ...
                for d in range(0, stepRange, step):
                    h = SurfaceRange(d, initialVelocityMPS, elevationAngleRadians, gravitationalConstant)
                    if debug == True: arcpy.AddMessage("d,h: " + str(d) + "," + str(h))
                    t = TimeToRange(h, initialVelocityMPS, elevationAngleRadians, gravitationalConstant)
                    if t > tMax: tMax = t
                    ...
                    if result.getOutput(0) == None or result.getOutput(0) == "NoData":
                        arcpy.AddWarning("Encountered NoData cell on surface. Using Zero elevation for cell\n") 
                        surfElev = 0.0
                        # Original tool stopped here, but let's contine with elevation = 0
            ...
            # special case:
            if (stepRange == 0): # if the range is zero e.g. projectile is shot straight up into the air
                if debug == True: arcpy.AddMessage("Zero range")
                x = longitudeOfObserver
                y = latitudeOfObserver
                z = elevationOfObserver
                tHalf = TimeToRange(maxHeight, initialVelocityMPS, elevationAngleRadians, gravitationalConstant)
            ...
            addRows.insertRow([arcpy.Polyline(pathArray,env.outputCoordinateSystem,True,True), \
                               maxHeight, dddRange, tMax,azimuthAngleDegrees ,elevationAngleDegrees, 
                               initialVelocityMPS])
...
if __name__ == "__main__":
    ...
    outFeature = computeTrajectory(inputFeature, initialVelocityMPS, elevationAngleDegrees, \
                       azimuthAngleDegrees, analysisType, inputSurface, outFeature)
